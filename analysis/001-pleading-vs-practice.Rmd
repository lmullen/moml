...
ggplot(subject_counts_by_year, aes(x = year, y = n, color = word)) +
  ...
  theme_minimal() +
  labs(title = "Change in counts of treatises with given words in LoC subjects")

ggplot(subject_counts_by_year, aes(x = year, y = percentage, color = word)) +
  geom_point(alpha = 0.25) + 
  geom_smooth(span = 0.1, se = FALSE) + 
  theme_minimal() +
  labs(title = "Change in percentage of treatises with given words in LoC subjects")
```
